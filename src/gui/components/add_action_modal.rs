...
fn show_add_action_modal_header(ui: &mut Ui, app: &mut MyApp) {
    match app.edit_action {
        Some(_) => {
            ui.heading(fl!(app.i18n_loader, "add-action-modal-heading-edit"));
        }
        None => {
            ui.heading(fl!(app.i18n_loader, "add-action-modal-heading-add"));
        }
    }
}
...
fn show_action_type_selector(ui: &mut Ui, app: &mut MyApp) {
    ComboBox::from_label("")
        .selected_text(label_for_action_type(&app.i18n_loader, &app.action_type))
        .show_ui(ui, |ui| {
            ui.selectable_value(
                &mut app.action_type,
                ...
                fl!(app.i18n_loader, "action-panel-left-click"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ...
                fl!(app.i18n_loader, "action-panel-right-click"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ActionType::Drag {
                ...
                fl!(app.i18n_loader, "action-panel-drag"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ActionType::Scroll {
                ...
                fl!(app.i18n_loader, "action-panel-scroll"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ActionType::KeyInput {
                ...
                fl!(app.i18n_loader, "action-panel-key-input"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ActionType::TextInput { text: "".to_string() },
                fl!(app.i18n_loader, "action-panel-text-input"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ActionType::Delay { millis: 500 },
                fl!(app.i18n_loader, "action-panel-delay"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ...
                fl!(app.i18n_loader, "action-panel-send-discord"),
            );
            ui.selectable_value(
                &mut app.action_type,
                ...
                fl!(app.i18n_loader, "action-panel-toggle-image-enable"),
            );
        });
}
    ...
    match &mut app.action_type {
        ActionType::LeftClick {
            ...
            ui.horizontal(|ui| {
                ...
                ui.checkbox(
                    use_matched_position,
                    fl!(app.i18n_loader, "action-panel-checkbox-use-matched-position"),
                );
            });
            ui.horizontal(|ui| {
                if ui
                    .button(fl!(app.i18n_loader, "action-panel-button-open-coordinate-picker"))
                    .clicked()
                {
        ...
        ActionType::Drag { start, end, duration_ms } => {
            ...
            ui.horizontal(|ui| {
                ui.label(fl!(app.i18n_loader, "action-panel-duration-ms"));
        ...
        ActionType::Scroll {
            ...
            ui.horizontal(|ui| {
                let s = if *delta == 120 {
                    fl!(app.i18n_loader, "action-panel-scroll-direction-option-up").to_string()
                } else {
                    fl!(app.i18n_loader, "action-panel-scroll-direction-option-down").to_string()
                };
                ComboBox::from_label("").selected_text(s.to_string()).show_ui(ui, |ui| {
                    ui.selectable_value(delta, 120, fl!(app.i18n_loader, "action-panel-scroll-direction-option-up"));
                    ui.selectable_value(delta, -120, fl!(app.i18n_loader, "action-panel-scroll-direction-option-down"));
                });
            });
            ui.horizontal(|ui| {
                ui.label("x: ");
        ...
        ActionType::KeyInput { keys } => {
            ...
            ui.separator();
            if ui.button(fl!(app.i18n_loader, "action-panel-key-add")).clicked() {
            ...
            ui.horizontal(|ui| {
                if let Some(index) = app.key_index {
                    ...
                    let key_type = match keys[index] {
                        KeyType::Down(_) => fl!(app.i18n_loader, "action-panel-key-type-down"),
                        KeyType::Up(_) => fl!(app.i18n_loader, "action-panel-key-type-up"),
                        KeyType::DownAndUp(_) => fl!(app.i18n_loader, "action-panel-key-type-down-and-up"),
                        KeyType::Delay(_) => fl!(app.i18n_loader, "action-panel-key-type-delay"),
                    };
                    ComboBox::new("key type", "").selected_text(key_type).show_ui(ui, |ui| {
                        ui.selectable_value(
                            &mut keys[index],
                            KeyType::Down(key_code),
                            fl!(app.i18n_loader, "action-panel-key-type-down"),
                        );
                        ui.selectable_value(
                            &mut keys[index],
                            KeyType::Up(key_code),
                            fl!(app.i18n_loader, "action-panel-key-type-up"),
                        );
                        ui.selectable_value(
                            &mut keys[index],
                            KeyType::DownAndUp(key_code),
                            fl!(app.i18n_loader, "action-panel-key-type-down-and-up"),
                        );
                        ui.selectable_value(
                            &mut keys[index],
                            KeyType::Delay(500),
                            fl!(app.i18n_loader, "action-panel-key-type-delay"),
                        );
                    });
                        ...
                        KeyType::Down(ref mut current) | KeyType::Up(ref mut current) | KeyType::DownAndUp(ref mut current) => {
                            ComboBox::new("key code", "").selected_text(current.to_string()).show_ui(ui, |ui| {
                                for k in [
                                    ...
                                    KeyCode::Num0, KeyCode::Num1, KeyCode::Num2, KeyCode::Num3, KeyCode::Num4, KeyCode::Num5, KeyCode::Num6, KeyCode::Num7, KeyCode::Num8, KeyCode::Num9,
                                    KeyCode::NumPad0, KeyCode::NumPad1, KeyCode::NumPad2, KeyCode::NumPad3, KeyCode::NumPad4, KeyCode::NumPad5, KeyCode::NumPad6, KeyCode::NumPad7, KeyCode::NumPad8, KeyCode::NumPad9,
                            ...
                            if let KeyCode::Custom(ref mut val) = current {
                                ui.horizontal(|ui| {
                                    ui.hyperlink_to(
                                        fl!(app.i18n_loader, "action-panel-key-custom-vk"),
        ...
        ActionType::SendDiscord {
            ...
            ui.label(fl!(app.i18n_loader, "action-panel-label-webhook-url"));

            ScrollArea::both().max_width(ui.available_width()).show(ui, |ui| {
                ui.text_edit_singleline(webhook_url);
            });
            ui.label(fl!(app.i18n_loader, "action-panel-label-message"));
            ui.text_edit_singleline(message);
            ui.checkbox(use_screenshot, fl!(app.i18n_loader, "action-panel-send-screenshot"));
        }

        ActionType::ToggleEnable { target, enable } => {
            ui.label(fl!(app.i18n_loader, "action-panel-label-target"));
            match target {
            ...
            let text = match enable {
                true => fl!(app.i18n_loader, "action-panel-enable-enabled"),
                false => fl!(app.i18n_loader, "action-panel-enable-disabled"),
            };
            ComboBox::new("enable", "").selected_text(text).show_ui(ui, |ui| {
                ui.selectable_value(enable, true, fl!(app.i18n_loader, "action-panel-enable-enabled"));
                ui.selectable_value(enable, false, fl!(app.i18n_loader, "action-panel-enable-disabled"));
            });
        }
    }
}
...
fn show_confirm_cancel_ui(ctx: &Context, ui: &mut Ui, app: &mut MyApp) {
    ui.horizontal_centered(|ui| {
        if ui.button(fl!(app.i18n_loader, "add-action-modal-button-confirm")).clicked() {
            if let Some(item_index) = app.selected_item_index {
                match app.edit_action {
                    ...
                    None => {
                        app.project.add_action(item_index, app.action_type.clone(), &app.i18n_loader);
        ...
        if ui.button(fl!(app.i18n_loader, "add-action-modal-button-cancel")).clicked() {
            app.show_action_modal = false;
            app.edit_action = None;
        }
...
fn label_for_action_type(loader: &FluentLanguageLoader, action_type: &ActionType) -> String {
    use ActionType::*;
    match action_type {
        LeftClick { .. } => fl!(loader, "action-panel-left-click"),
        RightClick { .. } => fl!(loader, "action-panel-right-click"),
        Drag { .. } => fl!(loader, "action-panel-drag"),
        Scroll { .. } => fl!(loader, "action-panel-scroll"),
        KeyInput { .. } => fl!(loader, "action-panel-key-input"),
        TextInput { .. } => fl!(loader, "action-panel-text-input"),
        Delay { .. } => fl!(loader, "action-panel-delay"),
        SendDiscord { .. } => fl!(loader, "action-panel-send-discord"),
        ToggleEnable { .. } => fl!(loader, "action-panel-toggle-image-enable"),
    }
}
