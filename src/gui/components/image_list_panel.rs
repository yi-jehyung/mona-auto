...
fn show_image_list_header(ui: &mut Ui, app: &mut MyApp) {
    ui.horizontal(|ui| {
        ui.label(RichText::new(fl!(app.i18n_loader, "image-list-panel-label")).strong());
        ui.add_enabled_ui(!app.is_automation_running, |ui| {
            if ui.button(fl!(app.i18n_loader, "image-list-panel-button-add-image")).clicked() {
                    ...
                    Frame::new().fill(bg).inner_margin(Margin::same(4)).show(ui, |ui| {
                        ui.horizontal(|ui| {
                            ui.checkbox(&mut item.enabled, "").context_menu(|ui| {
                                if ui.button(fl!(app.i18n_loader, "image-list-panel-context-rename")).clicked() {
                                    app.renaming = Some(index);
                                    app.rename_buffer = item.name.clone();
                                    ui.close_menu();
                                }
                                if ui.button(fl!(app.i18n_loader, "image-list-panel-context-delete")).clicked() {
                                    pending.delete = Some(index);
                                    ui.close_menu();
                                }
                                if ui.button(fl!(app.i18n_loader, "image-list-panel-context-move-up")).clicked() {
                                    pending.move_up = Some(index);
                                    ui.close_menu();
                                }
                                if ui.button(fl!(app.i18n_loader, "image-list-panel-context-move-down")).clicked() {
